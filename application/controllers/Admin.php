...
class Admin extends CI_Controller
{
    ...
    function employee_info()
    {
        ...
        $page_data['page_name']  = 'employee_info';
        $page_data['page_title'] = get_phrase('info_employee');
        $this->load->view('backend/index', $page_data);
    }


     function employee_profile($emp_id)
  {
    if ($this->session->userdata('admin_login') != 1) {
      redirect(site_url('login'), 'refresh');
    }
  ...
  function student_profile($student_id)
  {
    ...
    /***student_statement***/
    function student_statement($student_id = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        ...
        $page_data['page_name']  =   'student_report_card';
        $page_data['page_title'] =   get_phrase('statement_for') . ' ' . $student_name . ' (' . get_phrase('class') . ' ' . $class_name . ')&#x200E;';
        $page_data['student_id'] =   $student_id;
        $page_data['class_id']   =   $class_id;
        $this->load->view('backend/index', $page_data);
    }
	...
	function student_information($class_id = '')
	{
        ...
        //$page_data['page_title']    = get_phrase('student_information'). " - ".
                                            //$this->crud_model->get_class_name($class_id);
        $class_name   = $this->db->get_where('class' , array('class_id' => $class_id))->row()->name;
        $page_data['page_title'] = get_phrase('student_information')." / ".get_phrase($class_name);
		$page_data['class_id'] 	= $class_id;
		$this->load->view('backend/index', $page_data);
	}
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
        	...
        	if ($this->input->post('print_button')) {
        		//print_r($this->input->post('print_button'));exit();
        		$data['result'] = $this->input->post();
        		$this->load->view('backend/admin/student_add_form_print_view',$data);
        	} else 	{
        	$data['ar_name']         = $this->input->post('ar_name');
            $data['en_name']         = $this->input->post('en_name');
            $data['admission_status'] = $this->input->post('student-status');
            $data['building_id']    = $this->input->post('building_id');
            $data['nationality']         = $this->input->post('nationality');
            $data['status']         = $this->input->post('status');
            $data['class_id']       = $this->input->post('grade');
            $data['section_id']        = $this->input->post('section_id');
            $data['note']         = $this->input->post('note');
            $data['visaexpiry']         = $this->input->post('visaexpiry');
            $data['visastatus']         = $this->input->post('visastatus');
            $data['emirate']         = $this->input->post('emirate');
            //$data['area']         = $this->input->post('area');
            $data['joining_year']   = $this->input->post('joining_year');
            ...
            #$validation = email_validation($data['email']);
                ...
                if($this->input->post('grade') != null){
                  $data2['class_id']       = $this->input->post('grade');
                }
                ...
                move_uploaded_file($_FILES['pcsp']['tmp_name'], 'uploads/pcsp_image/' . $student_id . '.jpg');
                move_uploaded_file($_FILES['visa']['tmp_name'], 'uploads/visa_image/' . $student_id . '.jpg');
                move_uploaded_file($_FILES['emid']['tmp_name'], 'uploads/emid_image/' . $student_id . '.jpg');
        ...
        if ($param1 == 'do_update') {
            ...
            // if($validation == 1){
                ...
                $this->db->update('enroll' , array(
                    'class_id' => $data2['class_id'] , 'section_id' => $data2['section_id'] , 'roll' => $data2['roll']
                ));
    ...
    //     echo "</tr>";
    // }
            $sql = $this->db->insert('employee', $this->input->post());
            //echo $sql;
            //exit();
            $emp_id = $this->db->insert_id();

              
                move_uploaded_file($_FILES['personal_photo']['tmp_name'], 'uploads/employee_image/' . $emp_id . '.jpg');
        ...
        if($param1=='do_update')
        {
         ...
         redirect(site_url('admin/employee_info'), 'refresh');


          
    }
    function delete_employee($empid='') {
      $this->crud_model->delete_employee($empid);
    ...
    function print_employee($empid){
      $employee = $this->db->get_where('employee' , array('emp_id'=>$empid))->row();
      $page_data['employee'] = $employee;
      $this->load->view('backend/admin/employee_profile_print' , $page_data);       
    }

    function print_teacher($teacher_id){
      $employee = $this->db->get_where('teacher' , array('teacher_id'=>$teacher_id))->row();     
      $page_data['teacher'] = $employee;
      $this->load->view('backend/admin/teacher_profile_print' , $page_data);       
    }    
    ...
    //     echo "</tr>";
    // }
            $sql = $this->db->insert('teacher', $this->input->post());
            //echo $sql;
            //exit();
            $teacher_id = $this->db->insert_id();
            $validation = email_validation($data['email']);
            $data['password'] = sha1($this->input->post('password'));
            $newid = $this->input->post('Id_no');
            $this->db->update('teacher', $data, array('Id_no' => $newid));
            // if($validation == 1){
            //     $this->db->insert('teacher', $data);
            //     $teacher_id = $this->db->insert_id();
                move_uploaded_file($_FILES['personal_photo']['tmp_name'], 'uploads/teacher_image/' . $teacher_id . '.jpg');
                $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
        ...
        if ($param1 == 'do_update') {
            ...
            // $data['rexpiry_date'] = $this->input->post('rexpiry_date');
    ...
    function teacher_edit($teacher_id){
        ...
        $page_data['edit_data'] = $this->db->get_where('teacher' , array('teacher_id' => $teacher_id) )->result_array();
        $this->load->view('backend/index', $page_data);
    }
     ...
     function employee_edit($employee_id){
        ...
        $page_data['edit_data'] = $this->db->get_where('employee' , array('emp_id' => $employee_id) )->result_array();
        $this->load->view('backend/index', $page_data);
    }
    ...
    function classes($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            $data['name']         = $this->input->post('name');
            $data['teacher_id']   = $this->input->post('teacher_id');
            $query = $this->db->query('select ar_name from teacher where teacher_id = '.$data['teacher_id'].'');
            $query->result();
            $row = $query->row();
            if (isset($row))
            {
                    $data['tname'] = $row->ar_name;
            }
            $data['class_capacity']   = $this->input->post('class_capacity');
    ...
    function section($class_id = '')
    {
        ...
        $page_data['page_title'] = get_phrase('manage_divisions');
        $page_data['class_id']   = $class_id;
        $this->load->view('backend/index', $page_data);
    }
    ...
    function get_class_students($class_id)
    {
        ...
        foreach ($students as $row) {
            $name = $this->db->get_where('student' , array('student_id' => $row['student_id']))->row()->ar_name;
    ...
    function get_form_partial($lang_id){
        $data['lang'] = $lang_id;
        $this->load->view('backend/admin/form_partial', $data);
    }

    function get_gender_section($bldg_id){
        $sections = $this->db->get_where('class' , array(
            'building_id' => $bldg_id
        ))->result_array();
         echo '<option value="" selected disabled>' . get_phrase('select') . '</option>';
        foreach ($sections as $row) {
            echo '<option value="' . $row['class_id'] . '">' . get_phrase($row['name'] ) . '</option>';
        }        
    }
     ...
     function get_class_section($class_id)
    {
        ...
        //$classes = '<option value="" selected disabled>' . 'Select a section' . '</option>';
        echo '<option value="" selected disabled>' . get_phrase('select') . '</option>';
        foreach ($sections as $row) {
            //echo $row['name'];
            //$classes .= '<option value="' . $row['section_id'] . '">' . $row['name'] . '</option>';
    ...
    function get_transportation_section($emirate_id){
        $sections = $this->db->get_where('transport' , array(
            'emirate' => $emirate_id
        ))->result_array();
         echo '<option value="" selected disabled>' . get_phrase('select') . '</option>';
        foreach ($sections as $row) {
            echo '<option value="' . $row['transport_id'] . '">' . get_phrase($row['route_name'] ) . '</option>';
        }        
    }
    ...
    function attendance_update($class_id = '' , $section_id = '' , $timestamp = '')
    {
        ...
        foreach($attendance_of_students as $row) {
            ...
            $note = $this->input->post('note_'.$row['attendance_id']);
            $sms =  $this->input->post('sms_'.$row['attendance_id']);
            $student = $row['student_id'];
            $query2 = $this->db->query('select * from student where student_id = '.$student.'');
            $query2->result();
            $row2 = $query2->row();
            if (isset($row2))
            {
            ...
            $this->db->where('attendance_id' , $row['attendance_id']);
            $this->db->update('attendance' , array('note' => $note));
            $this->db->where('attendance_id' , $row['attendance_id']);
            $this->db->update('attendance' , array('sms' => $sms));
            
            if($sms == 1){
                $data['sms_num_1'] = $phone;
                $data['sms_message'] = 'تم نسجيل الطالب '.$name.'  غياب يوم '.$dow.' بتاريخ '.$date.'';
                $user="amrecgroup";
                $password="69440994";
                $mobilenumbers=$data['sms_num_1'];
                ...
                if (empty($ret)) {
                   // some kind of an error happened
                    ...
                    /*$this->session->set_flashdata('flash_message' , get_phrase('sms_sent'));
    ...
    function transport($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['bus_driver_number'] = $this->input->post('bus_driver_number');
            $data['bus_number'] = $this->input->post('bus_number');
            $data['number_of_passengers'] = $this->input->post('number_of_passengers');
            if ($this->input->post('description') != null) {
               $data['description']    = $this->input->post('description');
            }
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '') {
        ...
        $page_data['page_title']                = get_phrase('SMS_platform');
        $this->load->view('backend/index', $page_data);
    }
...
//                    $data2['roll']        = $row[1];
    ...
    function competition($param1="",$param2="",$param3="")
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        if ($param1 == 'create') {
            $data['name']     = $this->input->post('name');
            $query="SELECT MAX(id) as max FROM competition";
            $result=$this->db->query($query);
            if($result)
            {
                $res=$result->result_array();
                $index=$res[0]["max"]+1;
                //$data['link']    = "bilalbenrabah.net/index.php/admin/competition_form/".$index;
                $data['link']    = "localhost:8080/bilalcentre/index.php/admin/competition_form/".$index;
                
            }else
            {
                //$data['link']    ="bilalbenrabah.net/index.php/admin/competition_form/1";
                $data['link']    ="bilalbenrabah.net/index.php/admin/competition_form/1";
            }
        ...
        $page_data['edit_data'] = $this->db->get_where('competition', array(
            'id' => $param2
        ))->result_array();
        }
        if ($param1 == 'delete') {
        ...
        $page_data['competition']   = $this->db->get('competition')->result_array();
        $page_data['page_name']  = 'competition';
        $page_data['page_title'] = get_phrase('manage_competition');
        $this->load->view('backend/index', $page_data);
    }


    function competition_entry($param1="",$param2="",$param3="")
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        if ($param1 == 'create') {
           /* $data['name']     = $this->input->post('name');
            ...
            $this->db->insert('competition_entries', $this->input->post());    
            redirect(site_url('admin/competition_entry'), 'refresh');
        }       
        if ($param1 == 'do_update') {
        $data['name']        = $this->input->post('name');
        $data['link']       = $this->input->post('link');
        ...
        if ($param1 == 'delete') {
            $this->db->where('id', $param2);
            $this->db->delete('competition_entries');
            $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
            redirect(site_url('admin/competition_entry'), 'refresh');
        }
        $page_data['competition_entry']   = $this->db->get('competition_entries')->result_array();
        $page_data['page_name']  = 'competition_entry';
        $page_data['page_title'] = get_phrase('manage_competition_entries');
        $this->load->view('backend/index', $page_data);
    }

    function competition_form($id)
    {

        //$page_data['competition_entry']   = $this->db->get('competition_entries')->result_array();
        //$page_data['page_name']  = 'competition_form';
        //$page_data['page_title'] = get_phrase('competition_form');
        //$this->load->view('backend/index', $page_data);
        $this->load->view('backend/admin/competition_form');
    }
     function competition_formdemo($id)
    {
        //$page_data['competition_entry']   = $this->db->get('competition_entries')->result_array();
        //$page_data['page_name']  = 'competition_form';
        //$page_data['page_title'] = get_phrase('competition_form');
        //$this->load->view('backend/index', $page_data);
        $this->load->view('backend/admin/competition_formdemo');
    }
...
function visa($param1 = 'visa_home', $param2 = '', $param3 = ''){
        $page_data['message_inner_page_name']   = $param1;
        $page_data['page_name']                 = 'visa';
        $page_data['page_title']                = get_phrase('visa');
        $this->load->view('backend/index', $page_data);
    }

    function print($param1 = 'print_home', $param2 = '', $param3 = ''){
        $page_data['message_inner_page_name']   = $param1;
        $page_data['page_name']                 = 'print';
        $page_data['page_title']                = get_phrase('print');
        $this->load->view('backend/index', $page_data);
    }

    function print_student_class($param1 = 'print_student', $param2 = '', $param3 = ''){
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $this->load->view('backend/admin/print_student_class', $page_data);
    }

    function print_student_bus($param1 = 'print_student', $param2 = '', $param3 = ''){
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $this->load->view('backend/admin/print_student_bus', $page_data);
    }

    function print_teacher_view($param1 = '', $param2 = '', $param3 = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $this->load->view('backend/admin/print_teacher_view', $page_data);
    }

    function export_teacher_view($param1 = '', $param2 = '', $param3 = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $this->load->view('backend/admin/export_teacher_view', $page_data);
    }

    function print_employee_view($param1 = '', $param2 = '', $param3 = '') {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $this->load->view('backend/admin/print_employee_view', $page_data);
    }

function smsplatform($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(site_url('login'), 'refresh');
        $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
...
//exit();
            $data['sms_num_1'] = $this->input->post('sms_num_1');
...
//exit();   
            ...
            if($_POST['select_category'] == "students"){
                $query = $this->db->query('select phone from student');
                foreach ($query->result() as $row)
                {

                    $data['sms_num_1'] = $row->phone;
                    $data['sms_message'] = $this->input->post('sms_message');
                    $user="amrecgroup";
                    $password="69440994";
                    $mobilenumbers=$data['sms_num_1'];
            ...
            if($_POST['select_category'] == "teachers"){
                $query = $this->db->query('select tel from teacher');
                foreach ($query->result() as $row)
                {
            ...
            if($_POST['select_category'] == "employees"){
                $query = $this->db->query('select tel from employee');
                foreach ($query->result() as $row)
                {
            ...
            if($_POST['select_category'] == "parents"){
                $query = $this->db->query('select phone from parent');
                foreach ($query->result() as $row)
                {
        ...
        if ($param1 == 'tobus') {
            //var_dump($this->input->post());
//exit();   
            $trans = $this->input->post('select_bus');
            $query = $this->db->query('select phone from student where transport_id='.$trans.'');
                foreach ($query->result() as $row)
                {
        ...
        if ($param1 == 'tolevel') {
            var_dump($this->input->post());
//exit();   
            $trans = $this->input->post('select_level');
            $query = $this->db->query('select phone from student where class_id='.$trans.'');
                foreach ($query->result() as $row)
                {
