...
<div id="print">
    ...
    <tbody>
        <?php 
            ...
            $querybus = $this->db->query('select * from transport where transport_id='.$bus.'');
            $querybus->result();
            $rowbus = $querybus->row();
            if (isset($rowbus))
            {
                    $busno = $rowbus->bus_number;
                    $busroute = $rowbus->route_name;
            }
            $query = $this->db->query('select * from student where transport_id='.$bus.'');
            ?>
            <h3 style="font-weight: 100;">List of students in bus number <?php echo $busno; ?> in the route <?php echo $busroute; ?></h3>
            <?php
