	...
	<header class="row">
        ...
        <span>
          ...
          <a href="<?php echo site_url('admin/student_information/'.$enroll_info->row()->class_id);?>">
            <?php echo get_phrase('class').' - '.get_phrase($class_name).' | '. get_phrase('division').' - '.$section_name; ?>
          </a>
        </span>
      </center>
		</div>
    <div class="col-md-9">
      ...
      <!-- <li class="">
        <a href="#tab4" data-toggle="tab" class="btn btn-default">
          <span class="visible-xs"><i class="entypo-mail"></i></span>
          <span class="hidden-xs"><?php echo get_phrase('bus_info'); ?></span>
        </a>
      </li> -->
			<!-- <li class="">
				<a href="#tab4" data-toggle="tab" class="btn btn-default">
      ...
      <li class="">
        ...
        <?php
          // $basic_info_titles = ['name','parent', 'class', 'division', 'email', 'phone', 'address', 'gender', 'birthday', 'dormitory'];
          ...
          //  $basic_info_titles = ['name', 'class', 'division', 'email', 'phone', 'address', 'birthday'];
          // $basic_info_values = [$row['ar_name'], $class_name, $section_name, $row['email'], $row['phone'] == NULL ? '' : $row['phone'], $row['address'] == NULL ? '' : $row['address'], $row['birthday']];

          $basic_info_titles = ['id_no','name_in_arabic', 'name_in_english', 'nationality', 'joining_year', 'emirate', 'class', 'division', 'email', 'phone', 'birthday', 'orphan'];
          $basic_info_values = [$row['student_code'], $row['ar_name'], $row['en_name'], $row['nationality'], $row['joining_year'], $emirate, $class_name, $section_name, $row['email'], $row['phone'] == NULL ? '' : $row['phone'], $row['birthday'], $orphan];
        ...
        <table class="table table-bordered" style="margin-top: 20px;">
          ...
          <?php for ($i=0; $i < count($basic_info_titles) ; $i++) { ?>
            <tr>
              ...
              <td><?php echo get_phrase($basic_info_values[$i]); ?></td>
            </tr>
          <?php } ?>
          </tbody>
        </table>
        ...
        <?php } ?>
			...
			<div class="tab-pane" id="tab4">
				<?php
          $basic_info_titles = ['transport','driver_name','driver_contact','bus_number'];
          $basic_info_values = [$row['transport_id'] == NULL ? '' : $this->db->get_where('transport', array('transport_id' => $row['transport_id']))->row()->route_name,
          $row['transport_id'] == NULL ? '' : $this->db->get_where('transport', array('transport_id' => $row['transport_id']))->row()->driver_name,
          $row['transport_id'] == NULL ? '' : $this->db->get_where('transport', array('transport_id' => $row['transport_id']))->row()->bus_driver_number,
          $row['transport_id'] == NULL ? '' : $this->db->get_where('transport', array('transport_id' => $row['transport_id']))->row()->bus_number];
        ?>
        <table class="table table-bordered" style="margin-top: 20px;">
          <tbody>
          <?php for ($i=0; $i < count($basic_info_titles) ; $i++) { ?>
        ...
        <div>
          <table class="table table-bordered table-responsive">
            ...
            <tbody>
              <tr>
                <td>Student Image</td>
                <td><img src="<?php echo $this->crud_model->get_image_url('student', $student_id) ;?>" style="width: 60%;"/></td>
              </tr>

              <tr>
                <td>Passport Image</td>
                <td><img src="<?php echo $this->crud_model->get_image_url('pcsp', $student_id) ;?>" style="width: 60%;"/></td>
              </tr>

              <tr>
                <td>Emirates Id</td>
                <td><img src="<?php echo $this->crud_model->get_image_url('emid', $student_id) ;?>" style="width: 60%;"/></td>
              </tr>

              <tr>
                <td>Visa Photo</td>
                <td><img src="<?php echo $this->crud_model->get_image_url('visa', $student_id) ;?>" style="width: 60%;"/></td>
              </tr>
            </tbody>
          </table>
        </div>
